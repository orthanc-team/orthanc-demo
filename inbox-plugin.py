...
import typing
##
# This plugin adds API routes to perform custom actions:
# - after an upload in the inbox
# - when the custom button is clicked
# Ideally, we would have done 2 different plugins, but Orthanc can
# load only a single one...
##
...
def on_change(changeType, level, resource):
    ...
    if changeType == orthanc.ChangeType.ORTHANC_STARTED:
        ...
        # if a job is successful, and if this job is a DICOM store (or DW stow), we want to delete the study
        job_info = json.loads(orthanc.RestApiGet(f"/jobs/{resource}"))
        study_uid = None
        if job_info["Type"] == "DicomModalityStore":
            study_uid = job_info["Content"]["ParentResources"][0]
            orthanc.RestApiDelete(f"/studies/{study_uid}")
        

    elif changeType == orthanc.ChangeType.JOB_FAILURE:

        # if a job fails, we don't retry because the study has already been deleted
        # and the user has been notified
        orthanc.LogError(f"++++++++ job {resource} has failed, it could be a good idea to try to understand the cause...")
...
def on_post_validate_form(output, uri, **request):
    ...
    ## PatientEmail
    sender_name = form_fields['SenderName']
    if not sender_name:
        mark_invalid_field(result, 'SenderName', "")
    else:
        mark_valid_field(result, 'SenderName')
...
def on_post_inbox_commit(output, uri, **request):
    ...
    orthanc.LogInfo(f"INBOX: committing upload for {sender_name}")


    if len(uploaded_studies_ids) > 1:
        message = f"Wait while {len(uploaded_studies_ids)} studies are being processed"
    else:
        message = f"Wait while the study is being processed"
    ...
    #     # modify the study
    #     job = json.loads(orthanc.RestApiPost(f"/studies/{study_id}/modify", json.dumps({
    #         "Replace": {
    #             "AccessionNumber": patient_email
    #         },
    #         "Force": True,
    #         "Synchronous": False
    #     })))
    ...
    #     job_id = job['ID']
    #     orthanc.LogInfo(f"INBOX: Created processing job {job_id}")
    ...
    #                 orthanc.LogInfo(f"INBOX: Processing job {job_id} completed: {anonymized_study_id}")
    #         elif job['State'] == 'Failure':
    #             has_failed = True
    #             with store_lock:
    #                 commit_jobs[current_commit_id].has_failed = True
    #                 commit_jobs[current_commit_id].current_message = f"One of the study could not be processed.  Ref job: {job_id}"
    #         else:
    #             with store_lock:
    ...
    with store_lock:
        ...
        commit_jobs[current_commit_id].current_message = "All studies have been uploaded and processed."
